...
import { addTransactionCapability, aesEncryptCTR, bindWaitForConnectionUpdate, buildPairingQRData, bytesToCrockford, configureSuccessfulPairing, Curve, derivePairingCodeKey, fetchLatestWaWebVersion, generateLoginNode, generateMdTagPrefix, generateRegistrationNode, getCodeFromWSError, getCompanionPlatformId, getErrorCodeFromStreamError, getNextPreKeysNode, makeEventBuffer, makeNoiseHandler, promiseTimeout, signedKeyPair, xmppSignedPreKey } from '../Utils/index.js';
...
export const makeSocket = (config) => {
    ...
    const validateConnection = async () => {
        ...
        // The bundled default WA Web version can go stale, which makes WhatsApp
        // silently reject device linking even though a QR/pairing code is shown.
        // Unless the user explicitly pinned a version, refresh it here right
        // before the registration/login node is built.
        if (config.auto_fetch_latest_wa_version !== false) {
            try {
                const latest = await promiseTimeout(5000, (resolve, reject) => {
                    fetchLatestWaWebVersion({}).then(resolve).catch(reject);
                });
                if (latest?.version && latest.isLatest !== false) {
                    config.version = latest.version;
                    logger.info({ version: latest.version }, 'using latest WA Web version');
                }
            }
            catch (err) {
                logger.warn({ err }, 'failed to fetch latest WA Web version, falling back to bundled default');
            }
        }
        let node;
        if (!creds.me) {
    ...
    const requestPairingCode = async (phoneNumber, customPairingCode) => {
        const pairingCode = customPairingCode?.toUpperCase() ?? bytesToCrockford(randomBytes(5));
