...
export const makeGroupsSocket = (config) => {
    ...
    const { authState, ev, query, upsertMessage, executeUSyncQuery, signalRepository } = sock;
    ...
    // Resolve sisa participant yang masih @lid (server tidak kirim phone_number)
    // pakai cache mapping LID<->PN yang sudah dipelajari sebelumnya (history sync,
    // pesan dengan *Alt field, dsb) supaya `id`/`jid` yang keluar ke userland adalah JID.
    const resolveRemainingLidParticipants = async (metadata = {}) => {
        if (!Array.isArray(metadata.participants) || !metadata.participants.length) return metadata;
        const stillLid = metadata.participants
            .map(p => p.id || p.jid)
            .filter(id => isLidUser(id));
        if (!stillLid.length) return metadata;
        let pairs;
        try {
            pairs = await signalRepository.lidMapping.getPNsForLIDs([...new Set(stillLid)]);
        } catch {
            return metadata;
        }
        if (!pairs?.length) return metadata;
        const lidToPn = new Map(pairs.map(({ lid, pn }) => [lid, pn]));
        return {
            ...metadata,
            participants: metadata.participants.map(p => {
                const currentId = p.id || p.jid;
                const pn = isLidUser(currentId) ? lidToPn.get(currentId) : undefined;
                if (!pn) return p;
                return { ...p, id: pn, jid: pn, phoneNumber: pn, lid: p.lid || currentId };
            })
        };
    };
    const enrichGroupMetadataUsernames = async (metadata = {}) => {
        const resolved = await resolveRemainingLidParticipants(metadata);
        const normalized = normalizeGroupMetadataJids(resolved);
