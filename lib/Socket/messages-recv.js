...
export const makeMessagesRecvSocket = (config) => {
    ...
    const { userDevicesCache, ev, authState, ws, messageMutex, notificationMutex, receiptMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, registerSocketEndHandler, issuePrivacyTokens, fetchAccountReachoutTimelock, placeholderResendCache } = sock;
    const getLIDForPN = signalRepository.lidMapping.getLIDForPN.bind(signalRepository.lidMapping);
    /** this mutex ensures that each retryRequest will wait for the previous one to finish */
    const retryMutex = makeMutex();
    /** devicesMutex was destructured from sock but never created anywhere in the socket
     *  chain (same missing-mutex bug as processingMutex in messages-send.js), which
     *  threw "Cannot read properties of undefined (reading 'mutex')" whenever the device
     *  list cache was updated. Created locally, same pattern as retryMutex above. */
        ...
        try {
            ...
            if (!!alt) {
                ...
                if (altServer === 'lid') {
                    ...
                    // `primaryJid` here is a @lid and `alt` is its confirmed phone-number
                    // counterpart (that's exactly what the `else` branch above just verified
                    // by storing the mapping in that direction). Surface the phone-number jid
                    // as the primary identity on the message key — for both group participants
                    // and 1:1 chats — while keeping the raw @lid around in the *Alt field for
                    // anything downstream that still needs it (e.g. sending receipts back in
                    // the same addressing mode the message arrived in).
                    if (msg.key.participant) {
                        msg.key.participant = alt;
                        msg.key.participantAlt = primaryJid;
                    }
                    else if (msg.key.remoteJid) {
                        msg.key.remoteJid = alt;
                        msg.key.remoteJidAlt = primaryJid;
                    }
                }
            }
            else {
                // No *Alt field on this stanza (WA doesn't always send one, e.g. plain
                // groups/1:1 chats where the sender's PN was already learned earlier via
                // history sync, contacts, or a previous message that *did* carry an alt).
                // Fall back to the locally cached LID<->PN mapping so participant/remoteJid
                // still surface as a normal JID instead of leaking @lid to userland.
                const lookupAndSwap = async (kind) => {
                    const val = msg.key[kind];
                    if (!val || !isLidUser(val))
                        return;
                    const pn = await signalRepository.lidMapping.getPNForLID(val);
                    if (pn) {
                        msg.key[kind] = pn;
                        msg.key[`${kind}Alt`] = val;
                    }
                };
                await lookupAndSwap('participant');
                await lookupAndSwap('remoteJid');
            }
