...
/** Gets the duration (in seconds) of a video/audio file using ffprobe */
const getMediaDuration = async (path) => new Promise((resolve, reject) => {
    ...
    exec(cmd, (err, stdout) => {
        ...
        if (!isFinite(duration) || duration <= 0) {
            return reject(new Boom('could not determine media duration'));
        }
        resolve(duration);
    });
});
/** Picks a random timestamp (as an ffmpeg-friendly HH:MM:SS.ms string) somewhere in the middle of the video */
const getRandomVideoTimestamp = async (path) => {
    try {
        const duration = await getMediaDuration(path);
        // avoid the very first and very last moments, which are often black/blank/faded frames
        const minRatio = 0.1;
        const maxRatio = 0.9;
        const randomRatio = minRatio + Math.random() * (maxRatio - minRatio);
        const randomSeconds = Math.max(0, duration * randomRatio);
        return randomSeconds.toFixed(2);
    }
    catch {
        // fall back to a small fixed offset (rather than 0, which is frequently a black frame) if duration lookup fails
        return '00:00:01';
    }
};
...
export async function generateThumbnail(file, mediaType, options) {
    ...
    if (mediaType === 'image') {
        ...
        try {
            const randomTime = await getRandomVideoTimestamp(file);
            await extractVideoThumb(file, imgFilename, randomTime, { width: 252, height: 252 });
