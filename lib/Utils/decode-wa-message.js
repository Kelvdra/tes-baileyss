...
/**
 * Given a raw addressing identity and its counterpart (alt), pick the phone-number
 * (@s.whatsapp.net) JID as the "primary" one whenever it's known, keeping the LID
 * (if any) as the alt. This lets consumers of `key.remoteJid` / `key.participant`
 * always read a real jid — in groups and in DMs — instead of having to special-case
 * `@lid` addressed chats/participants themselves.
 */
const preferPnIdentity = (id, alt) => {
    if (id && alt && isLidUser(id) && isPnUser(alt)) {
        return { id: alt, alt: id };
    }
    return { id, alt };
};
...
export function decodeMessageNode(stanza, meId, meLid) {
    ...
    // Normalise the primary chat/participant identity to a real phone-number jid
    // whenever we know its @lid <-> @s.whatsapp.net counterpart, regardless of
    // whether this chat/group is currently PN-addressed or LID-addressed.
    const isGroupChat = isJidGroup(chatId);
    const chatIdentity = !isGroupChat ? preferPnIdentity(chatId, addressingContext.senderAlt) : { id: chatId, alt: undefined };
    const participantIdentity = isGroupChat ? preferPnIdentity(participant, addressingContext.senderAlt) : { id: participant, alt: undefined };
    const key = {
        remoteJid: chatIdentity.id,
        remoteJidAlt: chatIdentity.alt,
        remoteJidUsername: !isJidGroup(chatId)
            ? stanza.attrs.peer_recipient_username || stanza.attrs.recipient_username
            : undefined,
        fromMe,
        id: msgId,
        participant: participantIdentity.id,
        participantAlt: participantIdentity.alt,
